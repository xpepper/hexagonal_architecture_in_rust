...
use hexarch::outbound::sqlite_blog_repository::SqliteBlogRepository;
...
async fn main() -> anyhow::Result<()> {
    ...
    let blog_repository = SqliteBlogRepository::new(&config.database_url).await?;
    let prometheus = Prometheus::new();
    let email_client = EmailClient::new();
    let blog_service = Service::new(blog_repository, prometheus, email_client);
